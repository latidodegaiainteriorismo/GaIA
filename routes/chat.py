...
def chat():
    ...
    chunks           = search_knowledge(message, top_k=KNOWLEDGE_TOP_K)
    ...
    try:
        ...
        logger.warning(f'[CHAT] Rate limit Groq: retry_after={e.retry_after_seconds}s')
        minutos = max(1, round((e.retry_after_seconds or 60) / 60))
        gaia_text = (
            f'Necesito un momento de silencio — he hablado mucho hoy y toca pausar '
            f'unos {minutos} minuto{"s" if minutos != 1 else ""}. Vuelve en un rato, aquí estaré.'
        )
        resp = {'text': gaia_text, 'audio': None, 'rate_limited': True,
