...
# ── TTS de fragmentos sueltos ────────────────────────────────────────────────
#
# Sintetiza un fragmento de texto suelto, sin pasar por Groq ni por la base
# de datos. Lo usa el frontend para dos cosas:
#   1) "Escuchar" las preguntas del propio usuario (no traen audio del
#      backend como las respuestas de GaIA, así que se generan bajo demanda).
#   2) "Reproducir selección" — solo el fragmento de texto que el usuario
#      ha seleccionado a mano dentro de una respuesta ya mostrada.
#
# text_to_speech_fragment() trunca internamente a _TTS_FRAGMENT_MAX_CHARS
# (1500, cortando en frase completa) — no hace falta truncar aquí también.
